...
```
**编译产物**：
脚本执行成功后，您将在项目根目录下获得针对不同芯片架构编译的 3 款 `.zip` 和 3 款高质感 `.dmg` 镜像（推荐直接使用通用版 `BackDesk_v0.2.4_universal.dmg`）。

### 2. 安装与运行
1. 双击打开 `BackDesk_v0.2.4_universal.dmg`；
2. 将 `BackDesk.app` 图标拖入右侧的 `Applications`（应用程序）文件夹中；
