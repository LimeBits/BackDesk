...
BackDesk `v0.2.4` 是一次专门针对产品重命名以及文档完整性优化的维护更新。我们彻底清除了项目中所有遗留的 `ToDesktop` 字样，并将整个项目的包名、源码、文档和安装包完全统一为 `BackDesk`，同时重新打包发布了全新的 `v0.2.4` 版本安装包。
...
### 1. 🏷️ 全面重命名为 BackDesk
*   **重构说明**：将原应用名称 `ToDesktop` 彻底重构并更名为 `BackDesk`。
