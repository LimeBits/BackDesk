...
* **正确发布流程**：
  1. **打标签并推送**：
     ```bash
     git tag -a v0.2.5 -m "BackDesk v0.2.5"
...
### 3. 🔗 远程仓库更名后的 URL 重新指向
* **避坑说明**：若项目在托管平台上进行了重命名，本地的 Git 远程 URL 虽能被平台自动重定向，但仍应及时更新本地 remote 配置以规避未来的连接或鉴权风险：
  ```bash
  git remote set-url origin gitea-nas:brucetso/BackDesk.git
  ```
